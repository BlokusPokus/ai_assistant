...
class PerformanceOptimizer:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.resource_history: deque = deque(
            maxlen=1000)  # Keep last 1000 snapshots
    ...
    def collect_resource_snapshot(self) -> ResourceUsage:
        ...
        try:
            ...
            disk_io_read_mb = disk_io.read_bytes / \
                (1024 * 1024) if disk_io else 0
            disk_io_write_mb = disk_io.write_bytes / \
                (1024 * 1024) if disk_io else 0

            # Network I/O
            network_io = psutil.net_io_counters()
            network_io_sent_mb = network_io.bytes_sent / \
                (1024 * 1024) if network_io else 0
            network_io_recv_mb = network_io.bytes_recv / \
                (1024 * 1024) if network_io else 0
    ...
    def get_optimization_recommendations(self) -> List[PerformanceRecommendation]:
        ...
        try:
            ...
            self.logger.error(
                f"Error getting optimization recommendations: {e}")
            return []

    def optimize_worker_configuration(self, current_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if snapshot.cpu_percent > self.thresholds['cpu_warning']:
                ...
                for queue in ['ai_tasks', 'email_tasks', 'file_tasks', 'sync_tasks', 'maintenance_tasks']:
            ...
            if snapshot.memory_percent > self.thresholds['memory_warning']:
                ...
                optimal_config['worker_max_memory_mb'] = int(
                    memory_gb * 1024 * 0.6)  # 60% of total memory
                logger.info(
            ...
            with self.lock:
                ...
                for hour in range(1, forecast_hours + 1):
                    ...
                    predicted_memory = max(
                        0, min(100, memory_trend['slope'] * hour + memory_trend['intercept']))
                    ...
                    forecast['predictions'].append({
                        ...
                        'confidence': self._calculate_forecast_confidence(recent_snapshots)
                    })
        ...
        try:
            ...
            report = {
                ...
                'current_status': {
                    ...
                    'load_average': list(snapshot.load_average) if snapshot else [0, 0, 0]
                },
    ...
    def _calculate_averages(self, snapshots: List[ResourceUsage]) -> Dict[str, float]:
        ...
        try:
            ...
            averages = {}
            for field in ['cpu_percent', 'memory_percent', 'memory_available_gb',
                          'disk_io_read_mb', 'disk_io_write_mb', 'network_io_sent_mb',
                          'network_io_recv_mb', 'active_processes']:
    ...
    def _calculate_trends(self, snapshots: List[ResourceUsage]) -> Dict[str, Dict[str, float]]:
    ...
    def _identify_bottlenecks(self, snapshots: List[ResourceUsage]) -> List[str]:
        ...
        try:
            ...
            if latest.cpu_percent > self.thresholds['cpu_critical']:
                bottlenecks.append(
                    "Critical CPU bottleneck - system overloaded")
            ...
            if latest.memory_percent > self.thresholds['memory_critical']:
                bottlenecks.append(
                    "Critical memory bottleneck - system may crash")
            ...
            if avg_load > cpu_cores * self.thresholds['load_critical']:
                bottlenecks.append(
                    "Critical load bottleneck - system overloaded")
            ...
            if latest.disk_io_read_mb > 100 or latest.disk_io_write_mb > 100:
                bottlenecks.append(
                    "High disk I/O activity - potential I/O bottleneck")
    ...
    def _generate_recommendations(self, snapshots: List[ResourceUsage]) -> List[PerformanceRecommendation]:
