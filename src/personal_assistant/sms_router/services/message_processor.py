...
class MessageProcessor:
    ...
    def _clean_message(self, message: str) -> str:
        ...
        for abbr, full in abbreviations.items():
            ...
            cleaned = re.sub(r'\b' + re.escape(abbr) + r'\b',
                             full, cleaned, flags=re.IGNORECASE)
    ...
    def _calculate_spam_score(self, message: str) -> float:
        ...
        caps_ratio = sum(1 for c in message if c.isupper()) / \
            len(message) if message else 0
        if caps_ratio > 0.7:
            score += 0.3

        # Check for excessive punctuation
        punct_ratio = sum(1 for c in message if c in '!?.,;:') / \
            len(message) if message else 0
        if punct_ratio > 0.3:
            score += 0.2
    ...
    def _process_content(self, message: str, command_info: Optional[Dict[str, Any]]) -> str:
    ...
    def _analyze_sentiment(self, message: str) -> str:
        ...
        positive_words = ['good', 'great', 'excellent', 'awesome',
                          'love', 'like', 'happy', 'thanks', 'thank you']
        negative_words = ['bad', 'terrible', 'awful', 'hate',
                          'dislike', 'sad', 'angry', 'upset', 'frustrated']
