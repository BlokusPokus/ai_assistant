...
class SMSPerformanceMonitor:
    ...
    async def get_real_time_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if (current_time - self._last_cache_update) < self._cache_ttl and self._performance_cache:
        ...
        try:
            ...
            return {
                ...
                "summary": {
                    ...
                    "average_success_rate": self._calculate_average_success_rate(daily_performance),
                    "average_response_time": self._calculate_average_response_time(daily_performance),
                    "best_performance_day": self._find_best_performance_day(daily_performance),
                    "worst_performance_day": self._find_worst_performance_day(daily_performance)
                },
                "period": {
    ...
    async def check_sla_compliance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            response_time_compliant = response_time <= self.sla_thresholds["response_time_ms"]
            sla_checks["response_time"] = {
                ...
                "severity": "critical" if response_time > self.alert_thresholds["critical_response_time_ms"] else "warning" if response_time > self.alert_thresholds["warning_response_time_ms"] else "normal"
            }
            ...
            success_rate_compliant = success_rate >= self.sla_thresholds["success_rate_percent"]
            sla_checks["success_rate"] = {
                ...
                "severity": "critical" if success_rate < self.alert_thresholds["critical_success_rate_percent"] else "warning" if success_rate < self.alert_thresholds["warning_success_rate_percent"] else "normal"
            }
            ...
            availability_compliant = availability >= self.sla_thresholds["availability_percent"]
            ...
            error_rate_compliant = error_rate <= self.sla_thresholds["error_rate_percent"]
            sla_checks["error_rate"] = {
                ...
                "severity": "critical" if error_rate > (100 - self.alert_thresholds["critical_success_rate_percent"]) else "warning" if error_rate > (100 - self.alert_thresholds["warning_success_rate_percent"]) else "normal"
            }
            ...
            return {
                "sla_status": "compliant" if overall_compliance >= 100 else "non_compliant",
    ...
    async def generate_performance_alerts(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if current_metrics.get("total_messages", 0) > 0:
                ...
                current_success_rate = current_metrics.get(
                    "success_rate_percent", 100)
                historical_success_rate = historical_metrics.get(
                    "summary", {}).get("average_success_rate", current_success_rate)

                if historical_success_rate > 0 and current_success_rate < historical_success_rate - 5:
                    alert = {
                ...
                if historical_response_time > 0 and current_response_time > historical_response_time * 1.5:
                    alert = {
            ...
            alerts.sort(key=lambda x: severity_order.get(
                x.get("severity", "normal"), 0), reverse=True)
        ...
        try:
            ...
            return {
                ...
                "performance_summary": {
                    ...
                    "average_response_time": real_time_metrics.get("average_response_time_ms", 0),
    ...
    async def get_performance_recommendations(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            recommendations.sort(key=lambda x: priority_order.get(
                x.get("priority", "low"), 0), reverse=True)
    ...
    async def _calculate_real_time_metrics(self, recent_logs: List[SMSUsageLog]) -> Dict[str, Any]:
        ...
        try:
            ...
            success_rate = (successful_messages / total_messages *
                            100) if total_messages > 0 else 100
            ...
            today_logs = [
                log for log in recent_logs if log.created_at and log.created_at >= today_start]
            today_hourly_counts = {}
            for log in today_logs:
                if log.created_at and hasattr(log.created_at, 'hour'):
                    ...
                    hour = 0
                today_hourly_counts[hour] = today_hourly_counts.get(
                    hour, 0) + 1
            peak_users_today = max(
                today_hourly_counts.values()) if today_hourly_counts else 0
    ...
    def _analyze_performance_trends(self, daily_performance: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        # Response time trend
        first_time_avg = sum(day["average_response_time_ms"]
                             for day in first_half) / len(first_half) if first_half else 0
        second_time_avg = sum(day["average_response_time_ms"]
    ...
    def _analyze_performance_patterns(self, daily_performance: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Analyze performance patterns from daily data."""
        if not daily_performance:
            return {"patterns": "insufficient_data"}

        # Find best and worst performing days
        best_day = max(daily_performance,
                       key=lambda x: x.get("success_rate", 0))
        worst_day = min(daily_performance,
                        key=lambda x: x.get("success_rate", 0))
        ...
        for day in daily_performance:
            ...
            if weekday not in weekly_averages:
                weekly_averages[weekday] = {
                    "success_rates": [], "response_times": []}
            weekly_averages[weekday]["success_rates"].append(
                day["success_rate"])
    ...
    def _determine_overall_health(self, sla_checks: Dict[str, Any]) -> str:
        ...
        critical_count = sum(1 for check in sla_checks.values()
                             if check.get("severity") == "critical")
        warning_count = sum(1 for check in sla_checks.values()
    ...
    def _determine_system_status(self, health_score: float, alerts: List[Dict[str, Any]]) -> str:
        """Determine overall system status."""
        if health_score >= 90:
            return "healthy"
    ...
    def _calculate_average_success_rate(self, daily_performance: List[Dict[str, Any]]) -> float:
        """Calculate average success rate from daily performance data."""
        if not daily_performance:
            return 0.0

        total_rate = sum(day.get("success_rate", 0)
                         for day in daily_performance)
        return round(total_rate / len(daily_performance), 2)

    def _calculate_average_response_time(self, daily_performance: List[Dict[str, Any]]) -> float:
        """Calculate average response time from daily performance data."""
        if not daily_performance:
            return 0.0

        total_time = sum(day.get("average_response_time_ms", 0)
                         for day in daily_performance)
        return round(total_time / len(daily_performance), 2)

    def _find_best_performance_day(self, daily_performance: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    ...
    def _find_worst_performance_day(self, daily_performance: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
