...
class AgentIntegrationService:
        ...
        try:
            ...
            if self.agent_core:
                ...
                if self.tool_registry:
                    status['tool_count'] = len(self.tool_registry.tools) if hasattr(self.tool_registry, 'tools') else 0
    ...
    async def handle_agent_error(self, error: Exception, user_info: Dict[str, Any]) -> str:
        """
        ...
        try:
            ...
            logger.error(f"Agent error for user {user_info['id']}: {error_type}: {error}")
