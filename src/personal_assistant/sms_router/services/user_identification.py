...
class UserIdentificationService:
    ...
    async def identify_user_by_phone(self, phone_number: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if cached_user:
                logger.info(
                    f"User found in cache for phone: {normalized_phone}")
                return cached_user
            ...
            logger.warning(
                f"No user found for phone number: {normalized_phone}")
            return None

        except Exception as e:
            logger.error(
                f"Error identifying user by phone {phone_number}: {e}")
            return None
            ...
            try:
                ...
                result = await session.execute(
                    select(User.id, User.email, User.full_name,
                           User.is_active, UserPhoneMapping.phone_number)
    ...
    async def get_user_phone_numbers(self, user_id: int) -> list[str]:
        ...
        async with AsyncSessionLocal() as session:
            try:
                ...
                result = await session.execute(
                    select(UserPhoneMapping.phone_number)
    ...
    async def add_phone_mapping(self, user_id: int, phone_number: str,
                                is_primary: bool = False, verification_method: str = 'manual') -> bool:
        """
