...
class OAuthTokenService:
    ...
    async def store_tokens(
        self, db: AsyncSession, integration_id: int, tokens: Dict[str, Any], **kwargs
    ) -> List[OAuthToken]:
        """
        ...
        try:
            ...
            await db.rollback()
            raise OAuthTokenError(f"Failed to store tokens: {e}", "mixed", "store")

    async def get_valid_token(
        self, db: AsyncSession, integration_id: int, token_type: str
    ) -> Optional[OAuthToken]:
        """
    ...
    async def refresh_access_token(
        self, db: AsyncSession, integration_id: int, provider
    ) -> Optional[str]:
        """
        ...
        try:
            # Get refresh token
            refresh_token_obj = await self.get_valid_token(
    ...
    async def invalidate_token(
        self, db: AsyncSession, integration_id: int, token_type: str = "access_token"
    ) -> bool:
        """
    ...
    async def revoke_tokens(self, db: AsyncSession, integration_id: int) -> bool:
        """
        ...
        try:
            ...
            await db.rollback()
            raise OAuthTokenError(f"Failed to revoke tokens: {e}", "mixed", "revoke")
    ...
    async def cleanup_expired_tokens(self, db: AsyncSession) -> int:
        ...
        """
        try:
            query = delete(OAuthToken).where(OAuthToken.expires_at < datetime.utcnow())
