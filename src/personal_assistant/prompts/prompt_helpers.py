...
class PromptHelpers:
    ...
    def validate_note_context(
        conversation_history: list, last_tool_result: Any
    ) -> Dict[str, str]:
        ...
        for entry in conversation_history[-5:]:
            if entry["role"] == "user" and "note" in entry["content"].lower():
                ...
                match = re.search(r"note with ID: ([a-f0-9\-]+)", str(last_tool_result))
                if match:
    ...
    def identify_priority_tasks(
        step_count: int, last_tool_result: Any, note_topic: str
    ) -> str:
    ...
    def predict_user_needs(user_input: str, step_count: int, note_topic: str) -> str:
        ...
        if "schedule" in user_input.lower():
            ...
            return (
                f"Review {note_topic} results, make adjustments, or celebrate progress"
            )
        else:
    ...
    def format_memory_context(memory_context: List[dict]) -> str:
        ...
        for item in memory_context[:max_context_items]:
            ...
            if source == "ltm":
                formatted.append(f"💾 Long-term Memory ({content_type}): {content}")
            elif source == "rag":
                formatted.append(f"📚 Knowledge Base ({content_type}): {content}")
            else:
    ...
    def format_conversation_history(history: list) -> str:
        ...
        for i, entry in enumerate(history[-8:], 1):
            if entry["role"] == "user":
                ...
                if source == "ltm":
                    formatted.append(f"💾 LTM Memory ({content_type}): {content}")
                elif source == "rag":
                    formatted.append(f"📚 RAG Context ({content_type}): {content}")
                else:
                    formatted.append(f"💭 Memory ({content_type}): {content}")
    ...
    def classify_request_intent(user_input: str) -> str:
        ...
        if any(pattern in input_lower for pattern in simple_patterns):
            ...
            if input_lower.startswith("how ") and not any(
                greeting in input_lower
                for greeting in ["how are you", "how's it going"]
            ):
                return "information"
