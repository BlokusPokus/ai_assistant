...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column("tool_success", sa.String(length=10), nullable=True),
    ...
    op.create_table(
        ...
        sa.Column("original_role", sa.String(length=50), nullable=True),
    ...
    op.add_column(
        "memory_metadata", sa.Column("table_name", sa.String(length=50), nullable=True)
    )
    op.add_column(
        "memory_metadata", sa.Column("record_id", sa.Integer(), nullable=True)
    )
    op.add_column(
        "memory_metadata",
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=True),
    ...
    op.create_index(
        "idx_conversation_id", "conversation_states", ["conversation_id"], unique=True
    )
    op.create_index(
        "idx_conversation_user", "conversation_states", ["user_id", "created_at"]
    )
    op.create_index("idx_conversation_focus", "conversation_states", ["focus_areas"])
    op.create_index("idx_conversation_updated", "conversation_states", ["updated_at"])
    ...
    op.create_index("idx_message_type", "conversation_messages", ["message_type"])
    op.create_index(
        "idx_message_tool", "conversation_messages", ["tool_name", "tool_success"]
    )

    op.create_index(
        "idx_context_conversation_source",
        "memory_context_items",
    ...
    op.create_index("idx_context_type", "memory_context_items", ["context_type"])
    op.create_index("idx_context_focus", "memory_context_items", ["focus_area"])
    op.create_index("idx_context_timestamp", "memory_context_items", ["timestamp"])

    op.create_index(
        "idx_metadata_table_record", "memory_metadata", ["table_name", "record_id"]
    )
    op.create_index("idx_metadata_key_value", "memory_metadata", ["key", "value"])
    ...
    op.execute("UPDATE memory_metadata SET created_at = NOW() WHERE created_at IS NULL")
