...
class MemoryContextItem(Base):
    ...
    conversation_id = Column(String(255), ForeignKey(
        'conversation_states.conversation_id', ondelete='CASCADE'), nullable=False, index=True)
    ...
    @classmethod
    def from_memory_context_item(cls, conversation_id: str, item: dict, source: str = 'conversation'):
        """
        ...
        metadata_fields = ['id', 'source', 'content', 'relevance_score', 'context_type',
                           'original_role', 'focus_area', 'preference_type', 'timestamp']
        ...
        if not isinstance(content, str):
            if isinstance(content, dict):
                ...
                content = f"[{item.get('context_type', 'fact').upper()}] {source} context"
            else:
    ...
    @classmethod
    def from_focus_area(cls, conversation_id: str, focus_area: str, relevance_score: float = 0.8):
        """
    ...
    @classmethod
    def from_ltm_item(cls, conversation_id: str, ltm_item: dict, relevance_score: float = 0.7):
        """
    ...
    @classmethod
    def from_rag_item(cls, conversation_id: str, rag_item: dict, relevance_score: float = 0.6):
        """
