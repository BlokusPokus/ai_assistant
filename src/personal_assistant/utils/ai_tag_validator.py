...
class AITagValidator:
    ...
    def validate_and_correct_tags(
        ...
        try:
            ...
            if not ai_generated_tags or not ai_generated_tags.strip():
                ...
                return self._get_fallback_tags(content), [], "Used fallback tags due to empty AI response"

            # Clean and split the tags
            raw_tags = [tag.strip().lower()
                        for tag in ai_generated_tags.split(',') if tag.strip()]

            if not raw_tags:
                logger.warning(
                    "AI generated no valid tags after parsing, using fallback")
                return self._get_fallback_tags(content), [], "Used fallback tags due to no valid tags after parsing"
            ...
            if not valid_tags:
                logger.warning(
                    "No valid tags after correction, using fallback")
                return self._get_fallback_tags(content), invalid_tags, "Used fallback tags due to no valid tags after correction"
            ...
            return self._get_fallback_tags(content), [], f"Error during validation: {str(e)}"
    ...
    def validate_single_tag(self, tag: str) -> Tuple[bool, Optional[str]]:
        ...
        valid_tags, invalid_tags = validate_tags(
            [tag], enable_smart_fallback=True)
...
def validate_ai_generated_tags(
    ...
    Returns:
        Tuple of (valid_tags, invalid_tags, correction_explanation)
    """
    return ai_tag_validator.validate_and_correct_tags(ai_generated_tags, content, context)
