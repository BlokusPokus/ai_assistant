...
class StateOptimizationManager:
    ...
    async def optimize_state_for_saving(self, state: 'AgentState') -> 'AgentState':
        ...
        optimization_stats["optimization_steps"].append(
        ...
        if len(optimized_state.conversation_history) > self.config.max_conversation_history_size * 0.8:
            logger.info(
                "Step 4: Creating intelligent summaries for old content")
            optimized_state = self._create_intelligent_summaries(
                optimized_state)
            optimization_stats["optimization_steps"].append(
    ...
    def _create_intelligent_summaries(self, state: 'AgentState') -> 'AgentState':
        ...
        older_items = state.conversation_history[:-
    ...
    def _generate_intelligent_summary(self, items: List[dict]) -> dict:
        ...
        for tool_name, tool_items in tool_groups.items():
            success_count = len(
                [item for item in tool_items if "Error" not in str(item.get("content", ""))])
        ...
        return {
            ...
            "timestamp": datetime.now().isoformat(),
            "summary_metadata": {
                "tool_groups": {name: len(items) for name, items in tool_groups.items()},
    ...
    def _calculate_final_optimization_stats(self, original_state: 'AgentState',
        ...
        original_total = len(original_state.conversation_history) + \
            len(original_state.memory_context)
        optimized_total = len(
            optimized_state.conversation_history) + len(optimized_state.memory_context)
        ...
        if original_memory_size == 0:
            ...
            memory_reduction_percentage = (
                (original_memory_size - optimized_memory_size) / original_memory_size) * 100
    ...
    def get_optimization_report(self, original_state: 'AgentState',
                                optimized_state: 'AgentState') -> str:
        """
        ...
        conv_reduction = len(original_state.conversation_history) - \
            len(optimized_state.conversation_history)
        context_reduction = len(original_state.memory_context) - \
