...
class ConversationPatternLearner:
    ...
    def _analyze_communication_style(self, user_input: str, agent_response: str) -> Optional[dict]:
        ...
        if detected_styles:
            confidence = len(detected_styles) / \
                len(self.config.communication_style_indicators)
    ...
    def _analyze_topic_preferences(self, user_input: str) -> Optional[dict]:
        ...
        if detected_topics:
            confidence = len(detected_topics) / \
                len(self.config.topic_preference_keywords)
            if confidence >= self.config.memory_creation_confidence_threshold:
                return {
    ...
    def _analyze_time_patterns(self, user_input: str) -> Optional[dict]:
        ...
        time_keywords = ["morning", "afternoon", "evening",
                         "night", "today", "tomorrow", "weekend"]
        detected_times = [
            word for word in time_keywords if word in user_input.lower()]
...
class UserPreferenceLearner:
    ...
    async def learn_preferences(self, user_id: str, user_input: str, agent_response: str, tool_result: str = None) -> List[dict]:
        ...
        if tool_result:
            tool_preference = self._analyze_tool_preference(
    ...
    def _analyze_tool_preference(self, user_input: str, tool_result: str) -> Optional[dict]:
        ...
        if "error" in tool_result_str:
            ...
            tool_name = self._extract_tool_name_from_input(
                user_input) or "unknown_tool"
            return {
        ...
        success_indicators = ["success", "created",
                              "deleted", "updated", "found", "completed"]
    ...
    def _analyze_response_format_preference(self, user_input: str, agent_response: str) -> Optional[dict]:
        ...
        if detected_formats:
            confidence = len(detected_formats) / \
                len(self.config.response_format_indicators)
            if confidence >= self.config.memory_creation_confidence_threshold:
                return {
                    "type": "response_format_preference",
                    "content": f"User prefers {', '.join(detected_formats)} response format",
                    "confidence": confidence,
                    "tags": ["response_format", "preference", "style"] + detected_formats
                }
    ...
    def _analyze_timing_preferences(self, user_input: str) -> Optional[dict]:
        ...
        timing_keywords = ["quick", "fast", "slow",
                           "detailed", "brief", "comprehensive"]
        detected_timing = [
...
def get_learners(config: LTMConfig = None) -> tuple[ConversationPatternLearner, UserPreferenceLearner]:
    """Get both pattern and preference learners with shared configuration"""
    shared_config = config or LTMConfig()
    return ConversationPatternLearner(shared_config), UserPreferenceLearner(shared_config)
