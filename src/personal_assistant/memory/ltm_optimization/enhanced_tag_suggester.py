...
class EnhancedTagSuggester:
    ...
    def suggest_tags_for_content(
        ...
        try:
            ...
            content_tags, content_confidence = self._suggest_tags_from_content(
                content)
            ...
            if category:
                ...
                confidence_scores.extend(
                    [category_confidence] * len(category_tags))
            ...
            if user_context:
                ...
                confidence_scores.extend(
                    [context_confidence] * len(context_tags))

            # 5. Pattern-based suggestions
            pattern_tags, pattern_confidence = self._suggest_tags_by_patterns(
                content)
            ...
            if len(unique_tags) > max_tags:
                ...
                tag_confidence_pairs = list(
                    zip(unique_tags, unique_confidences))
                tag_confidence_pairs.sort(key=lambda x: x[1], reverse=True)
                unique_tags = [tag for tag,
                               _ in tag_confidence_pairs[:max_tags]]
                unique_confidences = [conf for _,
                                      conf in tag_confidence_pairs[:max_tags]]

            # Calculate overall confidence
            overall_confidence = sum(
                unique_confidences) / len(unique_confidences) if unique_confidences else 0.0
    ...
    def _suggest_tags_from_content(self, content: str) -> Tuple[List[str], float]:
        ...
        topic_keywords = {
            "work": ["work", "project", "meeting", "deadline", "office", "job", "career"],
        ...
        if any(word in content_lower for word in ["always", "usually", "typically", "habit", "routine"]):
            suggested_tags.append("habit")
            suggested_tags.append("pattern")

        if any(word in content_lower for word in ["learn", "understand", "figure out", "discover"]):
            suggested_tags.append("learning")

        # Communication-based suggestions
        if any(word in content_lower for word in ["explain", "tell me more", "detailed", "conversation"]):
        ...
        matched_keywords = sum(1 for keywords in topic_keywords.values()
                               if any(keyword in content_lower for keyword in keywords))
    ...
    def _suggest_tags_from_context(self, user_context: str) -> Tuple[List[str], float]:
        ...
        if any(word in context_lower for word in ["morning", "evening", "daily", "weekly"]):
            suggested_tags.append("routine")

        # Location-based suggestions
        if any(word in context_lower for word in ["home", "office", "work", "gym"]):
            suggested_tags.append(
                "personal" if "home" in context_lower else "work")
