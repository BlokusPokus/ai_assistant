...
class DatabaseOptimizer:
    ...
    def __init__(self, auto_start_monitoring: bool = False):
        self.performance_thresholds = {
            "slow_query_threshold_ms": float(
                os.getenv("DB_SLOW_QUERY_THRESHOLD_MS", "100")
            ),
            "table_bloat_threshold": float(
                os.getenv("DB_TABLE_BLOAT_THRESHOLD", "20.0")
            ),
            "index_usage_threshold": float(
                os.getenv("DB_INDEX_USAGE_THRESHOLD", "10.0")
            ),
            "connection_pool_efficiency": float(
                os.getenv("DB_POOL_EFFICIENCY_THRESHOLD", "80.0")
            ),
            "query_response_time_p95": float(
                os.getenv("DB_QUERY_RESPONSE_P95_THRESHOLD", "100")
            ),
        }
    ...
    async def analyze_table_performance(
        ...
        try:
            ...
            for stat in table_stats:
                ...
                bloat_percentage = (
                    (stat.dead_rows / max(stat.live_rows, 1)) * 100
                    if stat.live_rows > 0
                    else 0
                )
    ...
    async def analyze_index_usage(self, session: AsyncSession) -> List[IndexUsage]:
        ...
        try:
            ...
            for stat in index_stats:
                ...
                usage_percentage = (
                    (stat.scans / max(total_operations, 1)) * 100
                ...
                if stat.scans == 0:
                    ...
                    usage_percentage
                    < self.performance_thresholds["index_usage_threshold"]
                ):
    ...
    async def analyze_query_performance(
        ...
        try:
            ...
            for stat in query_stats:
                ...
                if (
                    stat.mean_time
                    < self.performance_thresholds["slow_query_threshold_ms"]
                ):
                    performance_category = "fast"
                elif (
                    stat.mean_time
                    < self.performance_thresholds["slow_query_threshold_ms"] * 2
                ):
    ...
    async def get_optimization_recommendations(
        ...
        try:
            ...
            for table in table_performance:
                if (
                    table.bloat_percentage
                    > self.performance_thresholds["table_bloat_threshold"]
                ):
                ...
                if (
                    not table.last_vacuum
                    or (datetime.now() - table.last_vacuum).days > 7
                ):
    ...
    async def get_performance_summary(self, session: AsyncSession) -> Dict[str, Any]:
        ...
        try:
            ...
            avg_table_performance = (
                sum(t.performance_score for t in table_performance)
                / len(table_performance)
                if table_performance
                else 0
            )
            total_index_size = sum(i.index_size_mb for i in index_usage)
            avg_index_efficiency = (
                sum(i.efficiency_score for i in index_usage) / len(index_usage)
                if index_usage
                else 0
            )
            ...
            summary = {
                ...
                "overall_performance": {
                    "score": round(
                        (avg_table_performance + avg_index_efficiency) / 2, 2
                    ),
                    "status": "healthy"
                    if avg_table_performance > 80 and avg_index_efficiency > 80
                    else "degraded",
                },
                "storage": {
                    "total_table_size_mb": round(total_table_size, 2),
                    "total_index_size_mb": round(total_index_size, 2),
                    "total_database_size_mb": round(
                        total_table_size + total_index_size, 2
                    ),
                },
                "tables": {
                    "count": len(table_performance),
                    "avg_performance_score": round(avg_table_performance, 2),
                    "high_bloat_count": sum(
                        1
                        for t in table_performance
                        if t.bloat_percentage
                        > self.performance_thresholds["table_bloat_threshold"]
                    ),
                },
                "indexes": {
                    ...
                    "unused_count": sum(
                        1 for i in index_usage if i.efficiency_score < 50
                    ),
                },
                "queries": {
                    ...
                    "avg_response_time_ms": round(
                        sum(q.avg_time_ms for q in query_performance)
                        / len(query_performance),
                        2,
                    )
                ...
                "connection_pool": {
                    ...
                    if pool_stats.utilization_percentage
                    > self.performance_thresholds["connection_pool_efficiency"]
                    else "inefficient",
                },
            }
