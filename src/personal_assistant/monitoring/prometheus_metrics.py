...
class PrometheusMetricsService:
    ...
    def update_system_metrics(self):
        ...
        try:
            ...
            self.system_network_io_bytes.labels(
                direction='sent').inc(network.bytes_sent)
            self.system_network_io_bytes.labels(
                direction='received').inc(network.bytes_recv)
    ...
    def record_sms_message(self, status: str, provider: str, duration: float = None, cost: float = None):
        """Record SMS message metrics."""
        self.sms_messages_total.labels(
            status=status,
            provider=provider
        ).inc()

        if duration is not None:
            self.sms_processing_duration_seconds.labels(
                provider=provider
            ).observe(duration)

        if cost is not None:
            self.sms_cost_total.labels(
                provider=provider
            ).inc(cost)

    def record_oauth_operation(self, provider: str, operation: str, duration: float, success: bool):
    ...
    def update_database_health(self, healthy: bool, connections: int = None, pool_utilization: float = None):
    ...
    def update_task_metrics(self, queue_lengths: Dict[str, int] = None, success_rates: Dict[str, float] = None):
        """Update task metrics."""
        if queue_lengths:
            for queue_name, length in queue_lengths.items():
                self.task_queue_length.labels(
                    queue_name=queue_name).set(length)
