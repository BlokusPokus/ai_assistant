...
class NotionPagesTool:
    ...
    async def create_note_page(
        ...
        try:
            ...
            logger.info(f"Created note page: {title} with ID: {note_page['id']}")
            return (
                f"Successfully created note page '{title}' with ID: {note_page['id']}"
            )
    ...
    async def read_note_page(self, page_identifier: str) -> str:
        ...
        try:
            ...
            if len(page_identifier.replace("-", "")) == 32:
                ...
                if not page_id:
                    return NotionErrorHandler.handle_notion_error(
                        ValueError(f"Page with title '{page_identifier}' not found"),
                        "read_note_page",
            ...
            return NotionErrorHandler.handle_notion_error(
                e, "read_note_page", {"page_identifier": page_identifier}
            )
    ...
    async def delete_note_page(self, page_id: str) -> str:
        ...
        try:
            ...
            return NotionErrorHandler.handle_notion_error(
                e, "delete_note_page", {"page_id": page_id}
            )
            ...
            for block in blocks.get("results", []):
                if block["type"] == "child_page":
                    ...
                    if query.lower() in page_title.lower():
                        results.append(f"Title match: {page_title} (ID: {page_id})")
                        continue
    ...
    async def create_link(self, source_page_id: str, target_page_title: str) -> str:
        ...
        try:
            # Find target page
            response = self.client.search(
                query=target_page_title, filter={"property": "object", "value": "page"}
            )
            ...
            logger.info(f"Created link from {source_page_id} to {target_page_title}")
            return (
                f"Successfully created link from source page to '{target_page_title}'"
            )
        ...
        try:
            ...
            for block in blocks.get("results", []):
                if block["type"] == "child_page":
                    ...
                    child_page_blocks = self.client.blocks.children.list(child_page_id)
                    ...
                    for child_block in child_page_blocks.get("results", []):
                        if child_block["type"] == "paragraph":
                            ...
                            if f"[[{page_title}]]" in content:
                                child_page = self.client.pages.retrieve(child_page_id)
                                child_title = (
                                    child_page.get("properties", {})
                                    .get("title", {})
                                    .get("title", [{}])[0]
                                    .get("plain_text", "")
                                )
            ...
            return NotionErrorHandler.handle_notion_error(
                e, "get_backlinks", {"page_id": page_id}
            )
