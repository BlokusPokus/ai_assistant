...
class NotionErrorHandler:
    ...
    NOTION_ERROR_PATTERNS = {
        "validation_error": [
            "title", "content", "page_id", "page_identifier", "query",
            "invalid", "required", "empty", "malformed", "format",
            "uuid", "not found", "missing"
        ],
        "permission_error": [
            "notion", "api", "key", "access", "permission", "unauthorized",
            "forbidden", "authentication", "scope", "integration"
        ],
        "resource_error": [
            "page", "block", "database", "not found", "404", "resource",
            "archived", "deleted", "unavailable", "parent", "child"
        ],
        "connection_error": [
            "connection", "network", "timeout", "unreachable", "refused",
            "http", "api", "server", "notion", "rate limit"
        ],
        "configuration_error": [
            "environment", "variable", "missing", "invalid", "config",
            "api key", "root page", "database id", "notion token"
        ]
    }

    @classmethod
    def create_notion_error_context(cls, error: Exception, method_name: str, args: dict) -> dict:
        """
        ...
        user_intent = cls.USER_INTENTS.get(
            method_name, "Perform Notion operation")
    ...
    @classmethod
    def get_notion_recovery_hints(cls, error_type: str, method_name: str, args: dict) -> list:
        """
        ...
        method_hints = notion_strategies.get(
    ...
    @classmethod
    def handle_notion_error(cls, error: Exception, method_name: str, args: dict) -> dict:
        """
        ...
        error_context = cls.create_notion_error_context(
