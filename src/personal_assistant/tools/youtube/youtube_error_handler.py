...
from ..error_handling import create_error_context, format_tool_error_response, classify_error
...
class YouTubeErrorHandler:
    ...
    YOUTUBE_ERROR_PATTERNS = {
        "validation_error": [
            "video_id", "channel_id", "playlist_id", "query", "max_results",
            "invalid", "required", "empty", "malformed", "format",
            "not found", "missing", "extract", "valid"
        ],
        "permission_error": [
            "youtube", "api", "key", "access", "permission", "unauthorized",
            "forbidden", "authentication", "scope", "quota", "exceeded",
            "403", "401", "developer", "api key"
        ],
        "resource_error": [
            "video", "channel", "playlist", "transcript", "not found", "404",
            "resource", "unavailable", "disabled", "captions", "no transcript",
            "private", "deleted", "removed"
        ],
        "connection_error": [
            "connection", "network", "timeout", "unreachable", "refused",
            "http", "api", "server", "youtube", "rate limit", "throttled"
        ],
        "configuration_error": [
            "environment", "variable", "missing", "invalid", "config",
            "youtube api key", "google api", "client library", "dependency"
        ],
        "quota_error": [
            "quota", "exceeded", "limit", "daily", "monthly", "cost",
            "billing", "usage", "allocation", "reset"
        ]
    }

    @classmethod
    def create_youtube_error_context(cls, error: Exception, method_name: str, args: dict) -> dict:
        """
        ...
        user_intent = cls.USER_INTENTS.get(
            method_name, "Perform YouTube operation")
    ...
    @classmethod
    def get_youtube_recovery_hints(cls, error_type: str, method_name: str, args: dict) -> list:
        """
        ...
        method_hints = youtube_strategies.get(
            error_type, {}).get(method_name, [])
    ...
    @classmethod
    def handle_youtube_error(cls, error: Exception, method_name: str, args: dict) -> dict:
        """
        ...
        error_context = cls.create_youtube_error_context(
