...
class ToolMetadataManager:
    ...
    def export_metadata(self, tool_names: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        if tool_names is None:
            ...
            tools_to_export = [name for name in tool_names if name in self.metadata_store]
    ...
    def import_metadata(self, metadata_data: Dict[str, Any]) -> List[str]:
        ...
        for tool_name, tool_data in metadata_data.get("tools", {}).items():
            try:
                ...
                self.logger.error(f"Failed to import metadata for tool {tool_name}: {e}")
    ...
    def get_metadata_summary(self) -> Dict[str, Any]:
        ...
        category_counts = {cat.value: len(tools) for cat, tools in self.category_index.items()}
        complexity_counts = {}
