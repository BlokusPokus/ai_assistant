...
class NotificationService:
    ...
    async def send_task_completion_notification(
        ...
        Returns:
            Notification result
        """
        print(f"🔍 BREAKPOINT 18: Entering send_task_completion_notification")
        print(f"🔍 BREAKPOINT 18: Task: {task.title}, Result: {result}")
        

        if not user_phone:
            print(f"🔍 BREAKPOINT 18: No phone number provided, using default")
            user_phone = os.getenv('TWILIO_TO_NUMBER')
            
        if not user_phone:
            print(f"🔍 BREAKPOINT 18: Still no phone number, returning error")
            return {
                'success': False,
        ...
        print(f"🔍 BREAKPOINT 19: Notification message created: {message}")

        print(f"🔍 BREAKPOINT 20: About to call send_sms_notification")
        result = await self.send_sms_notification(
            to_number=user_phone,
            message=message,
            task_title=task.title
        )
        print(f"🔍 BREAKPOINT 21: SMS notification result: {result}")
        return result
