...
class CalendarErrorHandler:
    ...
    CALENDAR_ERROR_PATTERNS = {
        "validation_error": [
            "date", "time", "subject", "start_time", "end_time", "duration",
            "invalid", "required", "empty", "malformed", "format", "timezone"
        ],
        "permission_error": [
            "microsoft graph", "api", "token", "access", "permission", "unauthorized",
            "forbidden", "authentication", "scope", "calendar.read", "calendar.write"
        ],
        "resource_error": [
            "event", "calendar", "not found", "404", "resource", "quota",
            "limit", "exceeded", "mailbox", "calendar id"
        ],
        "connection_error": [
            "connection", "network", "timeout", "unreachable", "refused", "http",
    ...
    @classmethod
    def create_calendar_error_context(cls, error: Exception, method_name: str, args: dict) -> dict:
        """
        ...
        user_intent = cls.USER_INTENTS.get(
            method_name, "Perform calendar operation")
    ...
    @classmethod
    def get_calendar_recovery_hints(cls, error_type: str, method_name: str, args: dict) -> list:
        """
        ...
        method_hints = calendar_strategies.get(
    ...
    @classmethod
    def handle_calendar_error(cls, error: Exception, method_name: str, args: dict) -> dict:
        """
        ...
        error_context = cls.create_calendar_error_context(
