...
async def _get_smart_ltm_context(
    user_id: int, user_input: str, focus_areas: list, logger
) -> str:
    """
    ...
    try:
        ...
        if optimized_context:
            logger.info(f"Smart LTM context generated: {len(optimized_context)} chars")
...
async def _get_fallback_ltm_context(
    ltm_tool, user_id: int, user_input: str, focus_areas: list, logger
) -> str:
    """
    ...
    try:
        ...
        if not focus_areas:
            ...
            logger.info(f"Generated focus areas from user input: {focus_areas}")

        # Get relevant LTM memories using the LTM tool
        logger.info(
            f"Calling ltm_tool.get_relevant_memories with context: {user_input[:100]}..."
        )
        ltm_context = await ltm_tool.get_relevant_memories(context=user_input, limit=10)
...
async def should_create_ltm_memory(
    user_input: str, response: str, importance_threshold: int = 7
) -> bool:
    """
...
async def create_ltm_memory_if_needed(
    ltm_tool,
    llm,
    logger,
    user_id: int,
    ...
    try:
        ...
        logger.info(f"Checking if LTM memory should be created for user {user_id}")
        ...
        if await should_create_ltm_memory(user_input, response):
            ...
            if any(
                word in ltm_content.lower()
                for word in ["urgent", "critical", "important"]
            ):
                importance_score = 9
            elif any(
                word in ltm_content.lower()
                for word in ["preference", "habit", "routine"]
            ):
                importance_score = 8
            ...
            result = await ltm_tool.add_memory(
                ...
                or "Automatically created from conversation",
            )
