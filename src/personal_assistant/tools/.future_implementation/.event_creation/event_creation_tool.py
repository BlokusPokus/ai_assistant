...
class EventCreationTool:
    ...
    async def create_event(self, user_input: str, user_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            validation_result = await self.ai_parser.validate_event_details(event_details)
    ...
    async def _create_base_event(self, event_details: EventDetails, user_id: int) -> Event:
        """Create the base event in the database."""
        # Calculate end time
        end_time = event_details.start_time + \
            timedelta(minutes=event_details.duration)
        ...
        try:
            ...
            db_pattern = DBRecurrencePattern(
                ...
                end_date=datetime.fromisoformat(
                    recurrence_pattern['end_date']) if recurrence_pattern.get('end_date') else None,
            ...
            for i, instance_datetime in enumerate(instances):
                instance_event = Event(
                    ...
                    timedelta(minutes=(base_event.end_time -
                              base_event.start_time).total_seconds() / 60),
                    source='sms_creation',
                    is_recurring=True,
    ...
    def _format_response_details(self, event_details: EventDetails, num_events: int) -> str:
