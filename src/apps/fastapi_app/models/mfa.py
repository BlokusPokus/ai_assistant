...
class MFAConfigurationCreateRequest(BaseModel):
    ...
    is_enabled: bool = Field(default=True, description="Whether MFA is enabled")


class MFAConfigurationUpdateRequest(BaseModel):
    """Request model for updating MFA configuration."""

    mfa_type: Optional[str] = Field(None, description="Type of MFA (totp, sms, email)")
    is_enabled: Optional[bool] = Field(None, description="Whether MFA is enabled")
...
class SecurityEventCreateRequest(BaseModel):
    ...
    event_type: str = Field(
        ..., min_length=1, max_length=50, description="Type of security event"
    )
    severity: str = Field(
...
class SecurityEventUpdateRequest(BaseModel):
    ...
    resolved: Optional[bool] = Field(None, description="Whether event is resolved")
