...
class ConversationStateCreateRequest(BaseModel):
    ...
    conversation_id: str = Field(
        ..., min_length=1, max_length=255, description="Unique conversation identifier"
    )
...
class ConversationStateUpdateRequest(BaseModel):
    ...
    step_count: Optional[int] = Field(None, ge=0, description="Current step count")
    last_tool_result: Optional[Dict[str, Any]] = Field(
        None, description="Last tool result"
    )
...
class ConversationMessageCreateRequest(BaseModel):
    ...
    conversation_id: str = Field(
        ..., min_length=1, max_length=255, description="Conversation identifier"
    )
    role: str = Field(
        ...,
        min_length=1,
...
class MemoryContextItemCreateRequest(BaseModel):
    ...
    source: str = Field(
        ..., min_length=1, max_length=50, description="Source of the context item"
    )
    ...
    focus_area: Optional[str] = Field(None, max_length=100, description="Focus area")
...
class ConversationSearchRequest(BaseModel):
    ...
    min_step_count: Optional[int] = Field(None, ge=0, description="Minimum step count")
    max_step_count: Optional[int] = Field(None, ge=0, description="Maximum step count")
