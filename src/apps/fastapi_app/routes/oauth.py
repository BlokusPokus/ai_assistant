...
@router.get("/providers", response_model=List[OAuthProviderInfo], dependencies=[Depends(security)])
async def get_supported_providers(
    ...
    try:
        ...
        for provider_name in oauth_manager.get_supported_providers():
            ...
            provider_info = {
                "google": {"display_name": "Google", "description": "Google services including Calendar, Drive, and Gmail"},
                "microsoft": {"display_name": "Microsoft", "description": "Microsoft 365 services including Outlook and OneDrive"},
                "notion": {"display_name": "Notion", "description": "Notion workspace and pages"},
                "youtube": {"display_name": "YouTube", "description": "YouTube videos and channels"}
            }.get(provider_name, {"display_name": provider_name.title(), "description": f"{provider_name.title()} integration"})
...
@router.post("/initiate", response_model=OAuthInitiateResponse, dependencies=[Depends(security)])
...
async def oauth_callback(
    ...
    provider: Optional[str] = Query(
        None, description="OAuth provider name (optional, will be extracted from state if not provided)"),
    db: AsyncSession = Depends(get_db),
    oauth_manager: OAuthManager = Depends(get_oauth_manager)
):
    """
...
@router.get("/integrations", response_model=List[OAuthIntegrationResponse], dependencies=[Depends(security)])
async def get_user_integrations(
    provider: Optional[str] = Query(None, description="Filter by provider"),
    active_only: bool = Query(
        True, description="Return only active integrations"),
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
    oauth_manager: OAuthManager = Depends(get_oauth_manager)
):
