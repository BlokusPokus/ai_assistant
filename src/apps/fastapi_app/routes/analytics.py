...
def create_permission_checker(resource_type: str, action: str):
    ...
    async def check_permission(
        ...
        try:
            ...
            logger.error(
                f"Error checking permission {resource_type}:{action}: {e}")
            raise HTTPException(
                status_code=500,
...
async def get_user_sms_analytics(
    ...
    try:
        ...
        # Get usage summary
        usage_summary = await analytics_service.get_user_usage_summary(user_id, time_range)

        # Get usage trends
        usage_trends = await analytics_service.get_user_usage_trends(user_id, time_range)

        # Get performance metrics
        performance_metrics = await analytics_service.get_user_performance_metrics(user_id, time_range)

        # Get message breakdown
        message_breakdown = await analytics_service.get_user_message_breakdown(user_id, time_range)
        ...
        raise HTTPException(
            status_code=500, detail="Failed to get SMS analytics")
...
async def get_user_sms_costs(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to get SMS cost analysis")
...
async def download_user_usage_report(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to generate usage report")
...
async def get_system_sms_analytics(
    ...
    try:
        ...
        # Get system metrics
        system_performance = await analytics_service.get_system_performance_metrics(time_range)
    ...
    try:
        ...
        for user_data in top_cost_users[:limit]:
            ...
            try:
                ...
                logger.warning(
                    f"Error getting analytics for user {user_id}: {e}")
                continue
        ...
        raise HTTPException(
            status_code=500, detail="Failed to get users SMS analytics")
...
async def get_historical_performance(
    ...
    try:
        ...
        historical_performance = await performance_monitor.get_historical_performance(time_range)
...
async def get_performance_alerts(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to get performance alerts")
...
async def _generate_csv_report(
    ...
    try:
        ...
        writer.writerow(
            ["Total Messages", usage_summary.get("total_messages", 0)])
        writer.writerow(
            ["Inbound Messages", usage_summary.get("inbound_messages", 0)])
        writer.writerow(
            ["Outbound Messages", usage_summary.get("outbound_messages", 0)])
        writer.writerow(
            ["Success Rate (%)", usage_summary.get("success_rate", 0)])
        writer.writerow(["Average Processing Time (ms)",
        ...
        writer.writerow(["Average Response Time (ms)", performance_metrics.get(
            "processing_time_metrics", {}).get("average_ms", 0)])
        writer.writerow(["Min Response Time (ms)", performance_metrics.get(
            "processing_time_metrics", {}).get("minimum_ms", 0)])
        writer.writerow(["Max Response Time (ms)", performance_metrics.get(
            "processing_time_metrics", {}).get("maximum_ms", 0)])
        writer.writerow([])
        ...
        writer.writerow(
            ["Total Cost", cost_breakdown.get("total_cost_usd", 0)])
        writer.writerow(
            ["Inbound Cost", cost_breakdown.get("inbound_cost_usd", 0)])
        writer.writerow(
            ["Outbound Cost", cost_breakdown.get("outbound_cost_usd", 0)])
        ...
        writer.writerow(["Total with Monthly Fees",
                         cost_breakdown.get("total_with_monthly_fees", 0)])
        writer.writerow(
            ["Cost per Message", cost_breakdown.get("cost_per_message", 0)])
...
async def _generate_json_report(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to generate JSON report")
