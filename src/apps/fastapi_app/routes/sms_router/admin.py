...
from personal_assistant.sms_router.models.sms_models import SMSUsageLog, UserPhoneMapping, SMSRouterConfig
...
async def get_phone_mappings(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to get phone mappings")
...
async def create_phone_mapping(
    ...
    is_primary: bool = Query(
        False, description="Whether this is the primary number"),
    verification_method: str = Query(
        "manual", description="Verification method"),
    db: AsyncSession = Depends(get_db)
):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to create phone mapping")
...
async def update_sms_config(
    ...
    try:
        ...
        result = await db.execute(
            select(SMSRouterConfig)
            .where(SMSRouterConfig.config_key == config_key)
        )
...
async def delete_phone_mapping(
    ...
    try:
        ...
        result = await db.execute(
            select(UserPhoneMapping)
            .where(UserPhoneMapping.id == mapping_id)
        )
        mapping = result.scalar_one_or_none()

        if not mapping:
            raise HTTPException(
                status_code=404, detail="Phone mapping not found")

        # Delete the mapping
        await db.delete(mapping)
        await db.commit()

        return {"message": "Phone mapping deleted successfully", "mapping_id": mapping_id}
        ...
        raise HTTPException(
            status_code=500, detail="Failed to delete phone mapping")
